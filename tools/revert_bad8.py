"""
revert_bad8.py — Revierte los 8 archivos con metadata incorrecta del lote 2.
Limpia los tags y restaura título limpio basado en el nombre de archivo.
"""
...
def main():
    print("="*55)
    print("  ↩️  Revirtiendo 8 archivos con metadata incorrecta")
    print("="*55)
    ok, fail = [], []
    for i, fname in enumerate(BAD_FILES, 1):
        print(f"\n[{i}/{len(BAD_FILES)}] {fname[:60]}...")
        local = TEMP_DIR / fname
        print("  ⬇️  Pull..."); 
        if not pull(fname, local):
            print("  ❌ FAIL_PULL"); fail.append(fname); continue
        print("  🧹 Limpiando tags...")
        if not revert(local):
            fail.append(fname); local.unlink(missing_ok=True); continue
        print("  ⬆️  Push...")
        if push(local, fname):
            print("  ✅ OK"); ok.append(fname)
        else:
            print("  ❌ FAIL_PUSH"); fail.append(fname)
        local.unlink(missing_ok=True)
    print(f"\n✅ Revertidos: {len(ok)} | ❌ Fallidos: {len(fail)}")
